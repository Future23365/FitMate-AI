...
- [x] 新增训练计划和单次编排草稿类型与 Zod Schema，包括 `WorkoutPlanDraft`、`WorkoutDayDraft`、`WorkoutPlanItemDraft`、`WorkoutRoutineDraft`。
...
- [x] 聊天页增加保存入口：长期计划草稿会转换为多套 `WorkoutRoutine` 并写入训练日历，单次训练编排草稿会直接转换并保存为一套 `WorkoutRoutine`。
