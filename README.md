...
- [ ] 后续处理：用户登录、会话管理和用户数据隔离。
- [ ] 后续处理：用户健身画像管理，包括目标、经验、伤病、器械、可用时间、训练偏好。
- [ ] 后续处理：聊天历史服务端持久化，替代 `localStorage`。
- [ ] 后续处理：训练计划服务端保存、编辑、版本管理和删除。
- [ ] 后续处理：训练日历服务端持久化，支持跨设备同步。
- [ ] 后续处理：训练执行记录落库，包括动作、组数、完成情况、训练反馈。
...
- [ ] 后续处理：所有用户私有数据查询基于 `userId` 做权限隔离。
...
- [ ] 后续处理：将训练编排、训练日历、训练执行从 `localStorage` 迁移到服务端数据。
