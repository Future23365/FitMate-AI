# AGENTS.md

## 项目概述

本项目是一个 AI 健身聊天助手。系统通过自然语言交互理解用户的健身目标、身体状态、训练限制、训练偏好和可用时间，并据此生成、调整和执行个性化训练计划。

完整架构说明见：`docs/architecture.md`。

## 技术栈

- Next.js App Router
- React
- TypeScript
- Tailwind CSS
- shadcn/ui
- Prisma
- PostgreSQL
- Zod
- OpenAI SDK / Vercel AI SDK / DeepSeek AI SDK

## 架构规则

- 保持 UI 层、API 层、AI 编排层、领域服务层和数据访问层之间的职责分离。
- 不要把业务逻辑直接写在 API Route 中。
- 不要在 UI 组件中直接调用数据库或 AI 服务。
- 确定性的健身业务规则应放在领域服务中。
- AI 编排逻辑应与数据校验、权限控制和持久化逻辑分离。

## AI 规则

- 模型生成的数据必须使用 Structured Outputs、Zod Schema 或 JSON Schema 进行结构约束。
- 所有模型输出在保存或执行前都必须经过服务端校验。
- Tool Calling 必须通过服务端函数执行。
- AI 不能直接写入未经校验的训练计划。
- AI 选择或生成的 exerciseId 必须经过数据库校验。
- 不要向 AI 工具暴露任意 SQL 查询能力。

## 健身领域规则

- LLM 负责理解和生成，后端服务负责规则约束和最终执行。
- 训练计划必须尊重用户的目标、伤病情况、可用器械、训练经验和可用时间。
- 保存到系统中的训练动作必须来自数据库。
- 对新手、疼痛用户或有伤病限制的用户，应避免高风险或高冲击动作。
- 不要提供医疗诊断或治疗建议。

## 数据规则

- PostgreSQL 是系统事实数据来源。
- 使用 Prisma 进行数据库访问。
- 所有用户输入必须使用 Zod 校验。
- 所有 AI 输出必须使用 Zod 或 JSON Schema 校验。
- 所有用户私有数据查询都必须基于 userId 做权限隔离。
- 优先使用结构化数据库过滤，再使用向量检索或语义排序。

## 安全规则

- 不要绕过权限校验。
- 不要信任客户端输入。
- 不要持久化未经校验的原始 AI 输出。
- AI 接口必须做限流和成本控制。
- 记录 AI 输出校验失败和工具调用失败日志。

## 开发规范

- 优先进行小范围、聚焦的修改。
- 不要重构与当前任务无关的代码。
- 不要在没有明确理由的情况下引入新的大型依赖。
- 在 API、AI 和数据库边界处保持明确的 TypeScript 类型。
- 修改数据结构时，同步更新相关测试、Schema 或校验逻辑。

## 前端UI样式规则

- 优先使用响应式布局，确保内容不会被剪裁，遮盖

## debug & 样式校验
- 开发者已自行启动dev服务，可以直接访问，不要轻易执行npm run dev
- 默认不用内置预览检查每个小改动。只有在我明确要求使用预览时，才使用内部预览，并默认切换到 PC 视口，推荐尺寸为 `1440x900`。

## 注释
- 为文件、函数、数据结构添加简短注释，方便理解代码

## UI 风格规范

项目采用偏 Material Design 3 的高级 AI 健身 SaaS 风格：干净、专业、明亮、略带运动科技感。整体以 Material Design 3 的 surface 色彩层级、清晰状态反馈、Material Symbols 图标和柔和交互为基础，但不要照搬 MD3 的过大圆角和移动端气质。界面应像一个可信赖的 AI 私教工作台，而不是普通后台系统，也不是黑红硬核健身 App。

### 视觉方向

- 设计底层参考 Material Design 3：使用 surface / panel / primary / outline / muted text 等层级化 token。
- 图标优先使用 Material Symbols Outlined。
- 保留 MD3 的清晰状态反馈、柔和 hover、浅色容器层级。
- 在 Web 端要更克制：动作卡片圆角控制在 12px，避免整体过软。
- 整体风格：高级健康科技 SaaS + AI 健身教练。
- 优先使用浅色背景、白色卡片、清晰边框、克制阴影，以蓝色作为主强调色。
- 界面保持冷静、专业，但通过训练数据、肌群重点、进度状态、AI 建议增强运动感。

### 设计 Token

- 页面背景：#F6F8FB
- 面板 / 卡片：#FFFFFF
- 浅色面板：#EEF2F6
- 主文字：#111827
- 次级文字：#667085
- 边框：#D7DDE5
- 主蓝色：#2459E6
- 主蓝色 Hover：#163FAF
- 浅蓝背景：#E6ECFF
- 危险 / 错误：#E5484D
- 成功浅底：#ECFDF3
- 警告浅底：#FFF7E6

### 圆角

- 动作卡片：12px
- 图片容器：8px
- 输入框 / 按钮 / 导航项：12px
- 大型摘要卡片：20px
- 标签 / Chip：8px，胶囊标签可用 999px
- 动作卡片不要使用过大的圆角。

### 阴影

- 默认卡片阴影：0 1px 2px rgba(16, 24, 40, 0.06), 0 8px 24px rgba(16, 24, 40, 0.06)
- Hover 阴影：0 16px 40px rgba(16, 24, 40, 0.10)
- 阴影要轻，不要做成厚重悬浮感。

### 布局

- 侧边栏宽度：256px
- 顶部栏高度：64px
- 主内容最大宽度：约 1180px
- 页面内边距：32px
- 网格间距：20px
- 页面结构建议：训练摘要区、AI 建议区、训练阶段分割线、动作卡片网格。

### 组件

- 动作卡片使用白色背景、1px 边框、12px 圆角、紧凑间距和清晰训练参数。
- 热身 / 恢复动作使用横向紧凑卡片。
- 正式训练动作使用带大图的卡片。
- 动作卡片可使用小标签，例如：复合动作、孤立动作、绳索、灵活性、有氧。
- 可以少量使用深色卡片展示训练摘要或 AI 洞察，用于增强高级感和对比度。
- 按钮默认使用圆角矩形，不要全部做成胶囊形。
- 图标按钮优先使用圆角方形。

### 动效

- 卡片 Hover：轻微上浮，例如 translateY(-4px)。
- 图片 Hover：轻微放大，例如 scale(1.05)。
- 抽屉动画要顺滑、快速。
- 避免夸张弹跳、重度渐变、过度毛玻璃和复杂动画。

### AI 健身体验

- AI 输出要尽量转化为可操作 UI，而不是只展示聊天文本。
- AI 推荐动作应使用结构化卡片，包含动作名称、推荐理由、目标肌群、难度和“加入训练”操作。
- 训练进行中页面应突出当前动作、组数 / 次数进度、休息计时器、AI 姿势提示和下一个动作预览。

### 避免

- 动作卡片圆角过大。
- 黑红硬核健身房风格。
- 大面积渐变或过度玻璃拟态。
- 厚重阴影。
- 颜色过多。
- 图标风格混用。
- 缺少健身上下文的普通后台布局。