...
- 本项目主要由 AI 协作生成代码，因此新增或修改核心代码时，应默认添加简短的意图注释，帮助后续维护者和 AI 理解业务边界。
...
- 普通 JSX、局部变量、简单赋值、明显的数组遍历、直观样式 class 不需要注释。
...
- 每次进行技术架构、实现逻辑、方案大调整的时候。必须在`docs/方案变更历史`文件夹中生成相关md文档。每个调整单独一份md文档。重点记录当前真实的问题。调整思路、方案，怎么做的，如果结果可以量化，则标注上量化的结果。该文档主要作用就是记录项目演变过程，细节不需要太细，以讲故事为标准。
- 涉及到核心链路改进时，如，调整llm提示词、调整推送逻辑等核心链路。则在`docs/项目演变历程.md`文档中追加，简要记录此次干了什么，为了解决什么问题，无需详细记录实现细节。
