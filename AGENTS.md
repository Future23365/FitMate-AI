...
- 在 merge / sync / 合并分支任务中，默认语义是保留双方分支的已提交有效工作。不能因为源分支没有某个文件，就判断当前分支新增文件是残留或应删除。
- merge 后如果测试失败，只能说明存在行为、测试边界或架构约束冲突；不得直接通过删除当前分支新增功能、测试、脚本、文档来让测试变绿。
- 修复 merge 后测试失败前，必须先确认失败文件的来源：
  - 使用 `git log -- <file>` 查该文件是哪个提交新增或修改的。
  - 判断它是当前分支的新功能、源分支显式删除的旧代码、还是生成文件/临时文件。
  - 如果它属于当前分支刚新增的有效功能，优先修正测试边界、迁移位置或补充说明，不得直接删除。
- 以下删除属于高风险删除，除非我明确要求删除/清理/移除/archive，否则必须先暂停并征求确认：
  - 删除超过 3 个已跟踪文件。
  - 删除整个目录。
  - 删除 `tests/**`、`manual-tests/**`、`scripts/**`、`openspec/changes/**`、`docs/**`、`package.json` scripts、配置文件。
  - 删除当前分支相对 merge source 新增的文件。
- 如果确实认为需要删除，必须先给出删除审查说明，包含：
  - 删除文件清单。
  - 每类文件为什么不再需要。
  - 证据来自哪个提交、测试、OpenSpec 或用户明确要求。
  - 是否存在“保留文件并调整测试/边界”的替代方案。
  - 等待我确认后才能执行删除。
- 不允许为了让 `npm test`、`typecheck` 或 lint 通过而删除有效测试套件、开发脚本、OpenSpec 文档或业务入口。测试失败优先视为合同或边界需要修正，而不是被扫到的文件应该删除。
- `git diff --name-status` 中出现 `D` 或 `R` 时，提交前必须单独说明这些删除/重命名是否属于本次任务范围。若删除不是用户明确要求或 OpenSpec 明确任务，不得自动提交。
本规则优先于“完成任务后自动 commit”。只要触发高风险删除确认条件，必须暂停，不得自动 stage / commit 删除结果。
...
- 注释重点解释“这个函数/对象/类型负责什么、为什么存在、在链路中的位置、使用边界是什么”，不要解释代码表面的执行步骤。
- 导出的函数、hook、service、工具函数、核心类型、Zod Schema、配置对象、映射表、领域模型，应添加一句简短中文注释，说明它在当前业务链路中的职责。
- 文件内的核心私有函数、复杂回调、状态转换函数、数据投影函数、校验函数、策略函数，也应添加简短注释，说明输入输出意图和业务边界。
- 重要声明对象、常量对象、配置片段、schema 字段集合、tool 定义、action 映射、UI 状态模型，应添加注释说明对象用途、消费方或稳定合同。
- 涉及 AI 编排、训练计划生成、动作选择、权限隔离、持久化、跨模块状态流的代码，即使实现不复杂，也应添加注释说明业务意图。
- 不需要给显而易见的局部变量、简单 JSX 片段、普通 `map` / `filter` 回调或纯展示样式逐行补注释；如果这些代码承载业务规则或用户可观察行为，则需要说明意图。
...
- 只有当本次改动属于重大架构调整、核心链路重构、核心领域模型变化、数据模型/持久化结构变化、生产 Agent 主链路变更、跨模块实现方案替换，或用户明确要求记录方案变更时，才在`docs/方案变更历史`文件夹中生成相关md文档。每个调整单独一份md文档。重点记录当前真实的问题如原方案为什么不合适、调整思路、关键改动、怎么做的。如果结果可以量化，则标注上量化的结果。该文档主要作用就是记录项目演变过程，细节不需要太细，以讲故事为标准。
- 普通 bug 修复、prompt/tool description 小修、测试补充、局部 UI/样式调整、局部类型修正、局部配置调整、OpenSpec 文档补充，不写入`docs/方案变更历史`，除非用户明确要求。
- `docs/项目演变历程.md`只记录对项目长期演进有持续影响的节点，例如架构调整、核心链路替换、核心实现逻辑重构、重要数据模型变化、生产 Agent 主链路变化、会影响后续开发方向的重要修复。
- 普通小修、小范围 prompt 合同补强、tool description 文案调整、测试补充、局部 bug fix 不追加`docs/项目演变历程.md`，除非用户明确要求。
- 如果是否需要记录到`docs/方案变更历史`或`docs/项目演变历程.md`不确定，先在回复中说明判断，不要自动写入这两个位置。
- 如需要在docs下生成文档，在要分配好归类文件夹，不要直接散落在docs目录下。
- 文档记录的时间精确到时分秒,以上海(UTC+8)时间为准

## 回答问题原则

当我询问某个现象、报错、异常、设计问题或实现问题的原因时，优先按照以下结构回答：

### 直接原因

说明导致当前现象发生的直接触发原因。

例如：哪个配置、代码、命令、环境、数据、调用链或状态直接造成了这个结果。

### 根本原因

说明为什么会出现这个直接原因。

需要尽量追溯到更底层的设计缺陷、流程问题、约束缺失、依赖问题、环境问题或认知误区。

如果根本原因无法根据当前信息确认，必须明确说明“不确定”，并列出需要补充的信息，禁止凭空猜测。

### 修复建议

分别给出：

* 针对直接原因的修复建议：优先解决当前问题，让现象立即恢复正常。
* 针对根本原因的修复建议：防止同类问题再次发生，包括代码结构、配置规范、校验机制、文档约束、测试用例或流程改进。

回答时不要只给结论，要说明判断依据。
如果问题涉及代码或配置，需要指出相关文件、字段、命令或调用链。
