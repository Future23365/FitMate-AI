...
## git提交规则
- 每次改动之后，无错误则自动提交一次commit，并添加中文提交说明。
...
- 优先复用项目现有模式、组件、hooks、utils、类型和样式约定；如果现有实现会导致明显的耦合、复杂适配或可读性下降，可以新增局部实现，但需要说明原因，并控制影响范围。
...
### 技能
- 当我说“看一下 log”、“看一下日志”时，读取项目根目录下的 `codex_logs/last_log.js`，这个文件是从dev log保存下来的相应步骤的内容，用于分析与预期不符的原因。