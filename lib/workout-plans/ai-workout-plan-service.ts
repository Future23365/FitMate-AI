...
export async function generateAiWorkoutPlanDraft(
  ...
  if (hasHighRiskHealthCondition(request.messages)) {
    return {
      ok: false,
      code: "high_risk_health_condition",
      message: "用户描述包含高风险健康情况，不能直接生成训练计划，应建议咨询医生或专业人士。",
    };
  }
  ...
  console.info("[ai-workout-plan] completed", {
    ...
    candidateCount: success.candidates.candidates.length,
  ...
  const content = await requestDeepSeekJson("intent_extraction", apiKey, [
    {
      ...
      content: [
        ...
        "不要输出 Markdown，不要解释。",
        "如果信息不足，请根据最保守且合理的默认值补齐：experience 默认 beginner，sessionMinutes 默认 30，weeklyFrequency 默认 3，数组字段默认 []。",
        "JSON 字段必须是：goal, experience, sessionMinutes, weeklyFrequency, equipment, injuryLimitations, preferences, avoidances。",
...
> {
  const candidatePayload = candidates.candidates.slice(0, maxModelCandidates).map(({ exercise }) => ({
  ...
  const content = await requestDeepSeekJson("draft_generation", apiKey, [
    {
      ...
      content: [
        ...
        "只能使用候选动作中的 exerciseId，禁止编造动作 ID。",
        "不能给出医疗诊断或治疗建议。",
        "如果用户有疼痛或伤病限制，计划必须保守，并在 safetyNotes 中提示停止疼痛动作并咨询专业人士。",
        "输出 JSON 字段必须是：title, goal, summary, weeklyFrequency, estimatedSessionMinutes, days, safetyNotes。",
        "days 每项字段必须是：title, focus, dayIndex, estimatedMinutes, items, safetyNotes。",
        "items 每项字段必须是：exerciseId, mode, sets, target, setRestSeconds, transitionRestSeconds, notes。",
        "mode 只能是 reps 或 duration。reps 的 target 表示次数，duration 的 target 表示秒数。",
        "组数、次数、时长和休息必须保守可执行。",
      ].join("\n"),
    },
    {
      ...
      content: JSON.stringify({
        intent,
        candidateExercises: candidatePayload,
