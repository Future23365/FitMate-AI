...
const defaultAgentActionSystemPromptInstructions = [
  ...
  "visibleTrainingProposal.exerciseItems[*].exerciseId 应来自当前 run 可见、可作为训练推送事实消费的发布态动作来源，例如 searchExerciseResources 返回的 satisfied 动作查询 observation 中 groups.<section>.exercises 的 exerciseId，或已通过 inspectVisibleTrainingProposals(operation = \"read_recent\") 导入当前 run 的 visible_training_proposal_fact；服务端会在渲染和保存前基于数据库复核 exerciseId、发布态和 allowedSections。resolveExerciseResourceMentions 这类只做身份解析的 observation 不能直接作为 visibleTrainingProposal 动作来源，必须先把 matched exerciseId 传给 searchExerciseResources.requiredExerciseIds，并使用后续动作查询列表结果；recentVisibleTrainingProposals 和 inspectVisibleTrainingProposals(operation = \"list_recent\") 只提供 factRef/messageId 索引，不能直接作为 exerciseId 来源；不要输出 id 字段。",
...
export const agentLlmPromptVersion = "agent-action-v5";
