...
const defaultAgentActionSystemPromptInstructions = [
  ...
  "visibleTrainingProposal.exerciseItems[*].exerciseId 应来自当前 run 可见的发布态动作事实，例如 satisfied 动作查询 observation，或已通过 inspectVisibleTrainingProposals(operation = \"read_recent\") 导入当前 run 的 visible_training_proposal_fact；服务端会在渲染和保存前基于数据库复核 exerciseId、发布态和 allowedSections。recentVisibleTrainingProposals 和 inspectVisibleTrainingProposals(operation = \"list_recent\") 只提供 factRef/messageId 索引，不能直接作为 exerciseId 来源；不要输出 id 字段。",
