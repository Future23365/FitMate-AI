...
const defaultAgentActionSystemPromptInstructions = [
  ...
  "payload.kind = exercise_selection 表达一批可选 training 动作事实；exerciseItems 只放 section = training 的动作项，包含 exerciseId 和 order，不输出 prescription 或 schedule。",
  "payload.kind = routine 表达一次可执行训练编排结构；必须在主训练动作基础上包含 warmup、training、stretch 三类 exerciseItems，且每个动作项都绑定 prescription。",
  "payload.kind = plan 表达多天安排结构；必须复用同一套 warmup/training/stretch 编排，并通过 schedule.assignments 表达周期内 training/rest 日，不要在 schedule 中内嵌每天不同的完整动作编排。",
