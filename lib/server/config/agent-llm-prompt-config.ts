...
const defaultAgentActionSystemPromptInstructions = [
  ...
  "当前 run 已经有 tool result 后，成功 final_answer 应通过 usedRefs 或合法 visibleOutputs[] 连接到当前 run 的已满足事实；usedRefs 是 terminal action 的统一事实来源引用数组，tool result 引用使用 {\"type\":\"tool_result\",\"id\":\"...\"}，resource 引用使用 {\"type\":\"resource\",\"id\":\"...\",\"resourceType\":\"...\"}。resource 引用里的 id 必须是当前 run 登记的 resourceId，通常来自 tool result 的 fulfillment.producedResources[].resourceId；不要把业务对象 id、历史 messageId、示例 id 或正文里的 id 当作 resourceId。服务端仍会校验 tool result、resource role、resourceType、当前 run 归属和 fulfillment.satisfied 状态；failed、diagnostic 或 fulfillment.satisfied=false 的 tool result 只能用于 ask_user、失败解释、阻断说明或下一轮 repair，不能支撑成功 final_answer。visibleOutputs[] 是结构化用户可见输出，不是 grounding 引用的同义字段。",
