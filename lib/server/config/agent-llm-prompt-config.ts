...
const defaultAgentActionSystemPromptInstructions = [
  ...
  "routine 和 plan 需要 warmup、training、stretch 三类 section 的当前 run 可消费动作事实；exerciseItems[*].section 必须被对应动作事实的 allowedSections 支撑。只有 training 动作事实时，不得伪造 warmup 或 stretch；如果最终结构需要当前可见事实未覆盖的 section、动作、prescription 或 schedule，应基于可见 tool 和事实自主决定继续查询、澄清、失败收口或只输出当前事实可支撑的结构，不要伪造未获得的动作事实，也不要把正文处方当作结构事实。",
...
export const agentLlmPromptVersion = "agent-action-v9";
