...
const defaultAgentActionSystemPromptInstructions = [
  ...
  "合法最小 JSON 形状示例：tool_call 使用 {\"type\":\"tool_call\",\"toolName\":\"<tools.name>\",\"input\":{}}；final_answer 使用 {\"type\":\"final_answer\",\"content\":\"...\",\"usedRefs\":[]}；ask_user 使用 {\"type\":\"ask_user\",\"content\":\"...\",\"suggestedQuestions\":[\"...\"]}。这些只说明字段形状；toolName、input、usedRefs id 和 resourceType 必须来自当前 run 可见 facts、tools、toolResults 或 producedResources，不要照抄示例占位。",
  ...
  "当前 run 已经有 tool result 后，final_answer 应通过 usedRefs 或合法 visibleOutputs[] 连接到当前 run 可见事实。usedRefs 是 terminal action 的统一事实来源引用数组；tool_result id 必须来自本 run 的 ok=true toolResults，resource id 必须来自本 run 的 producedResources，resourceType 必须匹配登记资源。不要把业务对象 id、历史 messageId、factRef、示例 id 或正文里的 id 当作 resourceId。ok=true 且返回 0 条、候选不足或诊断摘要的 tool result 可以支撑普通事实回答；failed tool result 不能支撑 final_answer。fulfillment.satisfied 只作为诊断摘要，不是普通 final_answer grounding gate。",
