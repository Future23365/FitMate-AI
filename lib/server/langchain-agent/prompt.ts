...
function buildDecisionExamplePromptRules() {
  return [
    ...
    "- 工具链路：如果当前可见事实已经提供可消费的动作和 section，可以直接用 submitVisibleTrainingProposal 承载周期计划；缺少 prescription 或 schedule 不通过动作库查询补齐，而由模型基于本轮目标、训练频率、单次时长、候选动作事实和保守训练编排构造；只有动作候选或 section 事实不足时，才查询动作库补齐候选；fitmate_final_response 输出解释正文。",
