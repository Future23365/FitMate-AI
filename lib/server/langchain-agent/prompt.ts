...
export function buildLangChainAgentSystemPrompt(input: BuildLangChainAgentSystemPromptInput = {}) {
  ...
  return [
    ...
    "- 普通训练知识、动作教学、注意事项、热身或拉伸方法、筛选结果说明，可以基于用户输入、上下文和成功工具事实直接通过 content 回答。",
    "- 当本轮要交付可展示、可后续引用或可继续调整的训练结构，例如训练动作集合、单次训练 routine 或多天训练 plan 时，正文 content 不能替代结构化训练结果。",
    "- 结构化训练结果必须通过当前 tool catalog 中的结构化训练收口工具提交并通过服务端校验；content 只负责解释推荐理由、注意事项、训练建议或补充说明。",
