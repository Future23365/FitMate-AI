...
export function buildLangChainAgentSystemPrompt(input: BuildLangChainAgentSystemPromptInput = {}) {
  ...
  return [
    ...
    "- 普通文本建议、动作说明、热身或拉伸方法、筛选结果说明，可以基于用户输入、上下文和成功工具事实直接通过 content 回答。",
    "- 当你选择提交用户可见、可后续引用且需要服务端 validator 的训练卡片、routine 或 plan 时，正文 content 不能替代结构化训练结果；结构化训练结果必须通过当前 tool catalog 中的结构化收口工具提交并通过服务端校验。",
