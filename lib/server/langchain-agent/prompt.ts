...
export function buildLangChainAgentSystemPrompt(input: BuildLangChainAgentSystemPromptInput = {}) {
  ...
  return [
    ...
    "",
    "训练请求范围判定：",
    "- 用户只给出训练目标、单次时长、场地、器械或强度条件，但没有表达周期、多天、每周、训练日、休息日、分化训练、全身覆盖、身体部位或具体目标肌群时，默认交付单次 routine。",
    "- 未指定具体肌群时，只表示没有该硬约束；不要把所有主要肌群、所有细分肌群或完整全身覆盖当成隐含待补齐条件。",
    "- 泛化训练请求只需要形成受时长约束的代表性训练课；动作选择应围绕当前 routine 的目标、时长、器械和可执行性收敛。",
    "- 只有用户明确要求全身覆盖、指定身体部位、指定分化训练、指定目标肌群，或当前可见候选无法组成任何可执行训练主体时，才把肌群拆分成动作库查询条件。",
    "",
...
function buildDecisionExamplePromptRules() {
  return [
    ...
    "- 默认查询口径：用户没有指定具体肌群、身体部位、分化训练或全身覆盖时，第一次动作候选查询应围绕当前 routine 的目标、场地 / 器械、时长和 suitabilities 获取代表性候选；不要先把目标拆成所有主要肌群查询。",
    "- 收口边界：broad training 候选已能组成受时长约束的主训练时，选择子集提交 routine；不要因为未指定肌群尚未逐个查询而继续补查。",
