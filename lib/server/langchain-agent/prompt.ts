...
export function buildLangChainAgentSystemPrompt(input: BuildLangChainAgentSystemPromptInput = {}) {
  ...
  return [
    ...
    "- 训练编排交付判据：training 候选已经能覆盖本轮主要训练目标时，可以从候选池选择子集构造主训练；不要为了逐个细分肌群补齐更多候选、扩大候选数量或排除未选候选而继续同类查询。",
    "- 训练编排交付判据：warmup 和 stretch 是辅助阶段；除非用户明确要求特定热身、拉伸、肌群覆盖或限制，否则已有可用辅助候选即可纳入编排，辅助阶段不要求每个目标肌群都有对应 primary 候选。",
    "- 训练编排交付判据：局部窄查询没有覆盖某些辅助肌群时，不等于整体 routine 或 plan 不可提交；如果 training 主体和可用辅助候选已经能形成可校验结构，应进入结构化训练收口。",
    "- 训练编排交付判据：用户只给出训练目标、单次时长、器械或场地条件，但没有表达多天、每周、周期、训练日或休息日安排时，优先交付单次 routine；只有目标明确需要跨天安排时才构造 plan。",
