...
export function buildLangChainAgentSystemPrompt(input: BuildLangChainAgentSystemPromptInput = {}) {
  ...
  return [
    ...
    "- 判断结构化训练结果是否还需要查询动作时，只看最终结构还缺不缺动作事实、section 事实或 prescription 事实；不要因为输出类型从 routine 派生成 plan，就默认重新查询动作。",
...
function buildDecisionExamplePromptRules() {
  return [
    ...
    "- 工具链路：如果当前可见事实已经提供可消费的动作、section 和 prescription，可以直接用 submitVisibleTrainingProposal 承载周期计划；只有缺少必要动作事实、section 或 prescription 时，才使用 searchExerciseResources 获取缺失候选；fitmate_final_response 输出解释正文。",
    "- 收口边界：已有事实足以组成周期计划时，停止同类查询；从已校验或已导入的 routine 派生成 plan 时，可以复用该 routine 的 exerciseItems、section 和 prescription，并补充 schedule。",
