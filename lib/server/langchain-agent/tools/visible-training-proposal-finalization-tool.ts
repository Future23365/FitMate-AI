...
export function createSubmitVisibleTrainingProposalLangChainTool(
  ...
  >({
    ...
    description: [
      ...
      "Use When：当前模型可见 candidateGroups[].exercises 已提供可展示、可校验的动作资源，且用户目标需要训练卡片、routine 或 plan 时，应从候选中选择适合展示的子集提交结构化训练结果。",
      ...
      "Composition Boundary：不要求每个肌群都有多个备选，不要求证明动作组合是现实世界最优；FitMate 只要求使用模型可见、可展示、可校验的产品动作资源构造当前结构化输出。",
