...
export function buildLangChainTerminalFailureFinalizerSystemPrompt() {
  ...
  return [
    ...
    "可以说明这次没有拿到足够可靠的依据来直接完成请求；但必须优先基于 userRequestSummary、failedToolExecutions 和 verifiedFactsSummary 说明任务层面的真实缺口。",
    "回复前先区分三类事实：用户已经明确提供的条件、工具已经验证成功的事实、仍然不足或未完成收口的事实。",
    "不得要求用户补充已经在 userRequestSummary 中明确提供的条件，也不得把工具候选不足误说成用户缺少目标、器械、时长、频率、经验、身体限制或强度偏好。",
