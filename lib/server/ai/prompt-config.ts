...
export const aiPromptModuleRegistry: Record<AiPromptModuleId, string> = {
  ...
  agent_tool_decision: [
    ...
    "用户没有明确提供可用器械、场地或已确认器械记忆时，动作推荐、routine 和 plan 都必须默认按无器械 / 自重训练处理，并在 searchExercises 的结构化 filters 中写入 homeRequirements=[\"no_equipment\"]；不要只在自由文本回复里说明无器械。",
    ...
    "普通回答、短回复回应、闲聊或没有明确执行目标时，应直接返回 answered 或 needs_clarification；不得为了完成形式而调用训练生成、验证、Policy 或保存工具。",
    "artifact 查看是只读工具链：需要展示刚才、最近或指定训练时，应通过 listRecentArtifacts / resolveArtifactReference / getArtifactPayload 读取 payload，并以 answered 引用 getArtifactPayload 的 usedToolResultIds 收口。",
    "只读 artifact 查看不需要 draftId、validationId、policyDecisionId 或 revisionId，也不得调用 saveConversationArtifactRevision。",
    "训练生成必须来自明确的生成 routine/plan 请求；训练修改必须来自明确的调整目标并读取目标 artifact payload；写入成功必须来自真实保存工具结果。",
    "当前产品未开放把训练保存到训练库、验证后保存或保存另一套训练的用户入口；assistantSuggestions 不得生成这类保存、验证后保存或未开放写入建议。",
    "assistantSuggestions 如表示下一步能力，必须尽量提供 targetOperation，例如 view_artifact、adjust_artifact、generate_from_recommendation、answer_followup 或 clarify_request；保存、验证后保存和未开放写入必须标为 unsupported_write / save_artifact / validate_and_save / write_artifact 或不要输出。",
    ...
    "长期 plan 的核心触发条件是目标、单次时长和每周频率；缺少器械或场地本身不能阻断生成，未指定时必须默认无器械 / 自重，并在 generatePlanDraft 的 intent 与 strategy.defaultAssumptions 中表达该默认。",
    "用户只表达可用器械如“有哑铃”时，默认把器械作为 training 主训练候选边界；不要默认要求 warmup/stretch 也必须使用该器械。只有用户明确要求全程同器械时，才把器械作为所有 section 的硬约束。",
    "调用 generateRoutineDraft 时，intent 必须是结构化 routine intent；若用户未说明经验水平可使用 experience=\"beginner\"，未指定器械时 equipment 必须表达无器械 / 自重，单次 routine 的 weeklyFrequency 可使用 1；若绑定 sourceArtifactId，requiredExerciseIds 必须来自该 artifact。",
  ...
  agent_final_result: [
    ...
    "artifact 查看读取到 routine/plan payload 后，可以用 answered 结束，并且 usedToolResultIds 必须引用成功的 getArtifactPayload tool result；该路径不得要求 draftId、validationId、policyDecisionId 或 revisionId。",
