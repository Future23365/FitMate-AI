...
// basicChatFixtureSourcePath 是基础 LLM 黑盒 JSON 用例的稳定来源，供命令行 runner 和 dev 审核页共享。
...
// parseBasicChatFixtureFromJson 将人工维护的 JSON fixture 转成 runner 和审核页都能消费的稳定内部模型。
...
// summarizeBasicChatFixture 为命令启动输出、审核页列表和统计摘要提供统一规模计算。
