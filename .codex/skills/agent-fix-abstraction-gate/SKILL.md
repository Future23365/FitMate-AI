...
这个 Skill 不替代 `agent-tool-change-governance` 或 `agent-prompt-contract-governance`：前者负责 Agent tool / LangChain runtime / production 能改哪里，后者负责模型实际可见输入；本 Skill 只负责判断修复方案抽象层级是否正确。
...
- 方案可能把具体用户短句、具体 `toolName`、字段组合、业务资源名、phrasing、`recent` / `current` / `latest` / `fromCard` / `forThisFlow` 这类当前 case 默认值写进通用 prompt、LangChain runtime、服务端生产规则或通用 wrapper / adapter 分支。
- 已有 diff、proposal、design 或 tasks 中出现服务端关键词规则、自然语言模板路由、phrasing 特判、具体业务 `toolName` 语义分支，或为单个业务 case 修改 LangChain runtime、tool wrapper 通用执行或 production response adapter。
...
- 业务实例：`visibleTrainingProposal`、`searchExerciseResources`、`inspectVisibleTrainingProposals` 等具体业务名只能作为 tool description、schema description、tool result summary、受控业务事实合同或测试样例出现。
...
- 服务端根据用户自然语言、关键词、正则、同义词、短句模板、历史摘要或具体 phrasing 改写 provider tool call、`toolName`、调用顺序、引用目标、调整目标或最终回答策略。
- 把具体业务实例名写成通用 Agent prompt 触发条件，而不是放在该 tool 的 description、schema description、tool result summary、受控业务事实合同或测试里。
- 为单个业务 tool 修改 LangChain runtime 主循环、model factory provider payload、tool wrapper 通用执行、structured final response / finalization tool 通用合同、production response adapter 或 `/api/chat` 主链路。
- 在 LangChain runtime、通用 wrapper 或 response adapter 中新增具体业务 `toolName` 分支，或让 tool handler 绕过 confirmation、permission、schema 校验、projection 或 final grounding。
...
- 业务 tool 局部说明：在对应 LangChain tool 的 `description`、schema description、examples 或 tool result summary 中说明该 tool 暴露什么事实、不能支撑什么事实。
- 受控业务事实 / projection：补 fact type、role、summary、consumable / diagnostic 边界、redaction 和最终输出可消费性。
- LLM repair / clarification：当结构冲突、字段缺失、引用不可用或结果不可执行时，进入工具失败反馈、terminal failure finalizer、澄清或拒绝，不把 intent 改写成另一个 provider tool call。
...
- 与改动范围相关的 prompt / tool description / schema description / tool result summary / 受控业务事实合同 / final grounding / tool-level 单测或 architecture boundary 检查。
- 若方案涉及具体业务名，说明该业务名属于 tool description、schema description、tool result summary、受控业务事实合同或回归测试，而不是通用语义规则。
