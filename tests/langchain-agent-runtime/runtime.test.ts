...
describe("LangChain Agent prompt", () => {
  it("describes native tool calling without old AgentAction contract terms", () => {
    ...
    expect(prompt).toContain("训练结构化交付前，必须先做模型自检");
    expect(prompt).toContain("A. 缺数据库动作事实");
    expect(prompt).toContain("B. 缺训练编排字段");
    expect(prompt).toContain("C. 缺用户必须确认的约束");
    expect(prompt).toContain("只有 A 类缺口才继续调用动作库查询工具");
    expect(prompt).toContain("当前没有可用于目标 section 的候选动作");
    expect(prompt).toContain("现有候选与用户明确硬约束冲突");
    expect(prompt).toContain("用户明确要求更多候选、替换候选、改变查询口径");
    expect(prompt).toContain("B 类缺口不能通过动作库查询补齐");
    expect(prompt).toContain("从候选池中选哪些动作、动作顺序、组数、次数、时长、休息、section 编排、prescription、schedule");
    expect(prompt).toContain("C 类缺口应追问一个阻塞问题");
    expect(prompt).toContain("新的查询会返回当前可见事实中不存在、且交付结构必需的数据库动作事实");
    expect(prompt).toContain("候选数量被截断、totalMatches 很大、仍可能存在更多动作、候选池不够理想");
    expect(prompt).toContain("需要生成 prescription / schedule，都不是继续查询动作库的理由");
    expect(prompt).toContain("训练编排候选消费边界");
