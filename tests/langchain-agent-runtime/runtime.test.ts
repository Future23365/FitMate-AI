...
describe("LangChain Agent prompt", () => {
  it("describes native tool calling without old AgentAction contract terms", () => {
    ...
    expect(prompt).toContain("本次 provider request 实际暴露的 tools schema");
    expect(prompt).toContain("未在本次 request 中暴露的工具不可调用");
    expect(prompt).toContain("历史消息、历史 tool_calls 或历史 tool result 中出现过的 toolName");
    expect(prompt).toContain("不代表该工具当前仍可调用");
    ...
    expect(prompt).toContain("需要展示数据库动作卡片、图片、routine、plan 或训练执行项时");
    expect(prompt).toContain("具体 exerciseId 必须来自当前模型可见数据库动作事实或受控业务事实");
    expect(prompt).toContain("失败、diagnostic 或不可消费结果只能用于解释边界、澄清或构造新的合法输入");
    expect(prompt).not.toContain("训练请求范围判定");
    expect(prompt).not.toContain("训练结构化交付前，必须先做模型自检");
    expect(prompt).not.toContain("A. 缺数据库动作事实");
    expect(prompt).not.toContain("B. 缺训练编排字段");
    expect(prompt).not.toContain("训练编排候选消费边界");
    expect(prompt).not.toContain("候选数量被截断、totalMatches 很大");
    ...
    expect(prompt).toContain('suitabilities = ["warmup", "training", "stretch"]');
    expect(prompt).toContain("如果用户没有指定肌群，不填写 muscles");
    ...
    expect(prompt).toContain("schedule 写在 payload 中");
    ...
    expect(prompt).toContain("具体 Markdown 边界以 fitmate_final_response.content schema description 为准");
    expect(prompt).not.toContain("适合聊天正文的 Markdown 子集");
    expect(prompt).not.toContain("raw HTML、Markdown 水平分割线");
  ...
  it("keeps final response content schema aligned with the chat markdown subset contract", () => {
    ...
    expect(contentProperty.description).toContain("不要输出未校验 JSON、NDJSON event 或工具调用参数");
    expect(contentProperty.description).toContain("content 只解释、提醒或总结已校验结构");
    expect(contentProperty.description).toContain("不要把正文当作结构化训练 payload");
    expect(contentProperty.description).toContain("不要在正文中补写 payload 未承载的动作、处方或 schedule 事实");
