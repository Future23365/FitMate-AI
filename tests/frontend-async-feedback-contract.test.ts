...
describe("frontend async feedback integration contract", () => {
  it("routes command-style workout and settings operations through async toast feedback", () => {
    ...
    const appSidebar = readProjectFile("components/app/app-sidebar.tsx");
    ...
    expect(actionComposerPage).toContain("action-composer-load-routine");
    ...
    expect(appSidebar).toContain("chat-history-delete");
    expect(localAuthProvider).toContain("正在重置本地用户...");
    expect(localAuthProvider).toContain("正在匿名登录...");
  });
  ...
  it("keeps visible read feedback delayed and background sync quiet", () => {
    ...
    expect(chatController).toContain("chat-history-load");
    expect(chatController).toContain("getChatInitialResponseToastUpdate");
    expect(actionComposerPage).toContain("action-composer-library-loading");
    expect(actionComposerPage).toContain("action-composer-routines-loading");
    expect(actionComposerPage).toContain("action-composer-preview-detail-loading");
