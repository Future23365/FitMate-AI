...
describe("agent LLM prompt configuration", () => {
  it("exposes a layered default AgentAction prompt and version", () => {
    ...
    expect(systemPrompt).toContain("{\"type\":\"tool_call\",\"toolName\":\"<tools.name>\",\"input\":{}}");
    expect(systemPrompt).toContain("{\"type\":\"final_answer\",\"content\":\"...\",\"usedRefs\":[]}");
    expect(systemPrompt).toContain("{\"type\":\"ask_user\",\"content\":\"...\",\"suggestedQuestions\"");
    expect(systemPrompt).toContain("不要照抄示例占位");
    ...
    expect(systemPrompt).toContain("tool_result id 必须来自本 run 的 ok=true toolResults");
    expect(systemPrompt).toContain("resource id 必须来自本 run 的 producedResources");
