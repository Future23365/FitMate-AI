...
describe("production LangChain tool catalog", () => {
  ...
  it("keeps tool descriptions in Chinese without old AgentAction contract terms", () => {
    ...
    expect(searchDescription).toContain("可展示、可校验、可用于训练卡片 / routine / plan 的动作资源候选");
    expect(searchDescription).toContain("不是现实世界动作全集检索、完整动作库存构建或专业最优动作筛选工具");
    expect(searchDescription).toContain("不要为了更专业、更完整、每个肌群更多备选或更纯净候选池继续查询");
    ...
    expect(descriptions).toContain("本 tool 只返回产品动作资源候选事实");
    expect(descriptions).toContain("不返回 prescription、schedule、routine 或 plan");
    expect(descriptions).toContain("缺口是动作取舍、处方或日程时");
    ...
    expect(descriptions).toContain("不是专业度、完整度、分页或继续深挖开关");
