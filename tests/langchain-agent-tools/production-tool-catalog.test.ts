...
describe("production LangChain tool catalog", () => {
  ...
  it("keeps tool descriptions in Chinese without old AgentAction contract terms", () => {
    ...
    expect(descriptions).toContain("用户没有指定具体肌群，且 broad query 已返回可用于当前 routine 的 training 候选");
    expect(descriptions).toContain("不要为了完整覆盖继续拆成胸、背、腿、肩、手臂、核心或等价全身肌群查询");
    ...
    expect(descriptions).toContain("未指定具体肌群的 routine 不要求覆盖所有主要肌群");
    expect(descriptions).toContain("当前候选能组成受时长约束的可执行训练主体时");
    ...
    expect(systemPrompt).toContain("训练请求范围判定");
    expect(systemPrompt).toContain("没有表达周期、多天、每周、训练日、休息日、分化训练、全身覆盖、身体部位或具体目标肌群时，默认交付单次 routine");
    expect(systemPrompt).toContain("未指定具体肌群时，只表示没有该硬约束");
    expect(systemPrompt).toContain("不要把所有主要肌群、所有细分肌群或完整全身覆盖当成隐含待补齐条件");
    expect(systemPrompt).toContain("受时长约束的代表性训练课");
    expect(systemPrompt).toContain("才把肌群拆分成动作库查询条件");
