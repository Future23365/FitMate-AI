...
- 将「保存全链路log」调整为双文件覆盖导出：
  - `codex_logs/ai_trace_log.js` 保存极简报告、模块摘要、关键 code/id、token usage、step 结构、`contentRef` 和 `detailRef` 引用。
  - `codex_logs/ai_trace_texts.jsonl` 保存脱敏后的长文本和结构化详情映射。
...
- `ai-trace-debugger`: 修改「保存全链路log」的导出合同，要求默认报告与映射文件分离，并提供可查找的 `contentRef` / `detailRef` 指引。

## Impact

- 影响代码：`components/dev/ai-trace-viewer.tsx`、`app/api/dev/ai-traces/route.ts`、`lib/server/agent-planners/model-adapters/model-adapter.ts`、`lib/server/agent-planners/model-adapters/deepseek-model-adapter.ts`。
- 影响测试：`tests/ai-trace-viewer.test.ts`、`tests/ai-trace-http.test.ts`、`tests/agent-core/adapter-llm-planner.test.ts`。
