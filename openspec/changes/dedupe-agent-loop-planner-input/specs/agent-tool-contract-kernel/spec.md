...
系统 SHALL 在不改变 `PlannerPort.decideNext(input)` TypeScript 外形的前提下，收敛当前 `PlannerInput` 中 `observations` 与 `toolResults` 的职责。`toolResults` SHALL 是成功 tool facts 的详细权威通道，`observations` SHALL 主要承载 repair、diagnostic、runtime boundary 和轻量索引。
...
- **AND** Runtime MUST 保持 `PlannerPort` 外部 type 兼容
...
- **WHEN** 后续新增或注册业务 tool
- **THEN** 该 tool 只要通过现有 `projection.model`、fulfillment 和 observation helper 输出安全投影
