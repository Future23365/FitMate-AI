...
本 change 采用小范围方案 A：不引入完整 `PlannerStateView`，不改 `PlannerPort` 外形，只把成功 tool result 的详细模型可见事实收敛到单一权威通道，先优化当前 Loop 最明显的输入噪声。
...
- DeepSeek adapter 或等价 model adapter 继续发送当前 `PlannerInput` 结构，不改变 `PlannerPort` type、不新增大状态机、不改 `AgentAction` schema。
...
- `agent-tool-contract-kernel`: Runtime 构造 Planner input 时必须区分“成功事实权威通道”和“repair / diagnostic observation 通道”，同时保持 `PlannerPort` 外形兼容。
...
- 影响模型实际可见输入：
  ...
  - `lib/server/agent-core/planner-port.ts` 的语义说明或注释
...
- 不影响：
  - `AgentAction` schema
  - `PlannerPort.decideNext(input)` 的 TypeScript 形状
  - ToolRegistry 注册模式
