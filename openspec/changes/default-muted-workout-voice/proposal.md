...
- 在右上角语音播报按钮附近展示一次轻量提示，说明可在此开启声音控制开关。
...
- `workout-session-voice-broadcast`: 调整首次进入默认静音、用户本地偏好持久化和右上角开启提示的规格。

## Impact

- 影响 `features/workouts/hooks/use-workout-voice-broadcast.ts` 中的语音播报偏好读取默认值。
- 影响 `features/workouts/components/workout-session-page.tsx` 中右上角播报按钮的默认状态和提示 UI。
