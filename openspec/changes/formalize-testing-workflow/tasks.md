...
- [ ] 1.2 新增最小测试配置，确保测试环境能解析 `@/*` 路径别名和当前 TypeScript 源码，并能发现 `expand-test-coverage` 新增测试文件。
...
- [ ] 2.5 将 `expand-test-coverage` 新增测试纳入同一 runner，不在本 change 中新增额外测试覆盖范围。
...
- [ ] 4.1 运行 `npm test`，确认现有迁移后的测试和 `expand-test-coverage` 新增测试会被自动发现并执行。
