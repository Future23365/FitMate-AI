...
- [ ] 4.1 将 `workout-persistence-service.ts` 拆分或重组为 routine、schedule、session result 三组职责
- [ ] 4.2 实现 routine 列表、详情、创建、更新、归档或删除逻辑，并保持 `userId` 权限隔离
- [ ] 4.3 实现 schedule 列表、详情、创建、状态更新、取消逻辑，并保持休息日和普通训练安排的边界清晰
- [ ] 4.4 实现训练完成服务方法，在事务中创建或更新 `WorkoutSessionResult` 并同步更新 `WorkoutSchedule.status = completed`
- [ ] 4.5 更新数据库记录到前端结构的 mapper，确保 schedule 返回 routine 快照、动作展示信息和训练执行所需数据
- [ ] 4.6 确保所有 AI 或客户端传入的 `exerciseId` 在保存 routine item 前经过数据库校验
...
- [ ] 9.1 运行 `openspec validate refactor-workout-data-model --strict`
- [ ] 9.2 运行 `npx prisma validate`
- [ ] 9.3 运行 `npm run db:generate`
- [ ] 9.4 运行 `npm test`
- [ ] 9.5 运行 `npm run typecheck`
- [ ] 9.6 运行 `npm run lint`
- [ ] 9.7 运行 `npm run build`，或记录无法运行的具体原因
