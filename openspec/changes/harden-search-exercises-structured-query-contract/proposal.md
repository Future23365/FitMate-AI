...
最新 AI Trace 中，用户要求“换一套没有器械的”，LLM 已经理解要查无器械上肢 routine，但 `searchExercises` 仍返回固定器械、杠铃、壶铃和绳索器械动作。根因不是单个词识别失败，而是当前 tool 没有把“LLM 想要 tool 完成什么结果”表达成可执行、可拒绝、可证明的结构化合同：LLM 只能在 `query`、`equipmentAvoided`、`candidateUse` 等零散字段里表达目标，tool 执行后也没有证明结果满足这些目标。

这个问题会影响所有 Agent tool：LLM 能理解用户意图并调用 tool，但如果 tool 的输入合同不足以表达目标，或结果合同不足以证明完成目标，就可能执行不了、执行偏、静默放宽，最后把半成品链路包装成成功结果。只修“无器械”或只修某个字段会继续留下同类架构问题。

## What Changes

- 建立统一的 Agent tool capability contract：每个 tool 必须声明自己能执行什么操作、需要哪些输入、怎样表达 LLM 期望的结果、如何严格执行、产出什么证据、哪些请求必须拒绝或澄清。
- 建立 `ToolRequest` / `ToolResult` 合同：LLM 每次调用 tool 时必须用结构化参数说明 operation、hard constraints、soft ranking hints、result requirements 和 projection；tool 成功时必须返回 `satisfied=true` 的执行证据，不能满足时必须返回结构化失败。
- LLM 负责理解用户语义并传入结构化参数；tool 负责严格执行这些参数并证明结果满足请求，不再依赖 prompt 文案、工具名或 runtime 特例猜测操作含义。
...
- `searchExercises` MUST 只接受白名单过滤字段、合法枚举和动作库真实 facet；执行型候选集合必须按这些字段做确定性 hard filter，并按 LLM 提交的 section 覆盖、候选数量、可用性等 `resultRequirements` 证明候选集合是否满足目标。
...
- 对当前工具无法严格表达但 LLM 常会需要的操作新增或调整 tool 能力，例如 artifact 引用解析使用 `resolveArtifactReference`，用户记忆查询使用结构化 `queryUserMemory`，避免让 `searchArtifacts` / `getUserMemory` 用粗糙参数假装完成精确语义操作。
...
- `agent-tool-capability-contract`: 定义 Tool-first 架构中每个 Agent tool 的能力声明、输入合同、严格执行、拒绝条件和执行证据。
  - 该能力同时定义 `ToolRequest` / `ToolResult` 的统一语义：LLM 必须显式声明想让 tool 产出什么结果，tool 必须证明已经产出该结果或明确失败。
...
- 影响 `searchArtifacts`、`getUserMemory` 等读工具的能力说明和参数边界；如现有参数无法严格执行 LLM 可能提出的操作，必须收窄描述、补结构化参数、拆出 `resolveArtifactReference` / `queryUserMemory` 或返回不支持。
- 影响 Agent trace / 黑盒报告中的工具 diagnostics 展示，用于说明执行型候选集合按哪些结构化字段过滤、哪些字段无效、候选是否满足查询证明。
- 需要补充单元测试、Agent 工具测试、tool request replay 测试和真实模型黑盒用例，覆盖每类 tool 的严格执行、拒绝条件、执行证据，以及无器械、器械要求、风险排除、难度、section 覆盖、routine 补动作、artifact 引用解析、memory 查询和 patch 替换等场景。
