...
- [ ] 3.5 通过 `diagnostics` 表达 `exercise_name_not_found`、`exercise_name_section_conflict`、`exercise_name_filter_mismatch` 或等价稳定 code。
...
- [ ] 4.3 从 production tool catalog、模型可见 manifest、schema/examples 和导出列表中移除 `resolveExerciseResourceMentions`。
...
- [ ] 6.4 使用 `rg` 定向检查 production 模型可见合同中不再暴露 `q` 和 `resolveExerciseResourceMentions`。
