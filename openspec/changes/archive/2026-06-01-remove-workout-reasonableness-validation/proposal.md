...
- 调整目标时长和周频率校验：只有当字段来源表明用户明确提出或确认该约束时，服务端才可将明显不一致作为契约失败；默认值、LLM 推断值或旧模型生成字段不得作为 hard fail 依据。
- 保留用户明确避免动作和具体训练禁忌的 hard fail 边界；未主动提出或未确认的伤病限制不得作为 hard fail 依据。
