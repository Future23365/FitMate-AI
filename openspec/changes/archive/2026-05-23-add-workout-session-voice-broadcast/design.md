...
- 每个动作步骤开始前先播报动作名和目标量，等动作提示播报完成后，再播报“3，2，1，开始”；倒计时结束后才启动该动作步骤的训练计时。
...
当 `/training` 载入有效 timeline 后，页面先进入第一个动作的准备阶段，不立即启动训练计时器，也不立即开始 3 秒倒计时。语音开关开启时，先播报动作准备句，例如“第一组动作，平板支撑，60 秒”；动作准备句的 `SpeechSynthesisUtterance` 完成后，再按秒播报“3，2，1，开始”；倒计时归零后才启动当前动作步骤的正式计时。

后续每次进入新的动作步骤时，也必须先进入准备阶段，播报动作名和目标量，例如“下一组，俯卧撑，30 个”；动作准备句播报完成后，再播报“3，2，1，开始”，最后开始该动作计时。休息步骤仍按休息计时执行；当休息结束并切到下一个动作步骤时，再触发下一个动作的准备播报和倒计时。准备阶段需要暂停训练主计时器、热量估算和动作进度，避免用户还没开始动作时训练已经消耗时间。
