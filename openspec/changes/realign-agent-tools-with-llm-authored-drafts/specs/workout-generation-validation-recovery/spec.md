...
训练生成校验恢复 SHALL 只提出 LLM repair、重新搜索动作、澄清或阻断建议。服务端 recovery MUST NOT 自动补写 routine section、plan day、exercise item、sets、target、rest 或训练语义字段并把结果当成成功 draft。

#### Scenario: Routine draft 校验失败
- **WHEN** LLM-authored routine draft 因缺少 section、动作来源越界或结构字段缺失而校验失败
- **THEN** recovery MUST 返回可执行的 repair guidance
- **AND** Agent MAY 让 LLM 修复 draft 或重新调用纯搜索工具
- **AND** 服务端 MUST NOT 自动生成缺失 section 或替换动作后返回成功

#### Scenario: Plan draft 校验失败
- **WHEN** LLM-authored plan draft 因 schedule、训练日、动作来源边界或结构字段缺失而校验失败
- **THEN** recovery MUST 返回可执行的 repair guidance
- **AND** Agent MAY 让 LLM 修复 draft 或重新调用纯搜索工具
...
当 LLM-authored draft 未通过校验时，系统 SHALL 保留失败诊断和搜索结果 evidence，但不得登记为可保存资源。
