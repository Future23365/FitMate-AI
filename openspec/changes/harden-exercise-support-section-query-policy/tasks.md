...
- [ ] 1.1 完成 Agent 修复方案抽象层级门禁审查，确认本 change 的失败证据只作为测试样例，生产规则只落在 `searchExerciseResources` tool 合同、repository policy、observation projection 和 trace summary。
- [ ] 1.2 确认任务分类为已有业务 tool 合同调整 / Agent tool bug 修复，不新增业务 tool，不修改 Agent core contract。
- [ ] 1.3 记录允许触碰模块：`searchExerciseResources` tool bundle、Exercise repository 查询构造、tool output schema、model observation、trace summary、tool-level tests、manifest / schema summary tests 和必要文档。
- [ ] 1.4 记录禁止触碰模块：orchestrator 主循环、`PlannerPort`、Executor、Policy Guard、ResourceStore、Resource Contract Validator、Response Renderer、`/api/chat` production route、服务端用户原文关键词分流、pgvector / RAG 基础设施。
- [ ] 1.5 如修改 manifest、schema description、examples、repair feedback、observations 或 compressed tool results，使用 `agent-prompt-contract-governance` 检查模型可见合同；本 change 只定义执行边界。

## 2. Repository 查询策略

- [ ] 2.1 为 Exercise resource 查询构造 section-aware hard filter policy，`training` 使用 `training` policy，`warmup` / `stretch` 使用 `support_section` policy。
- [ ] 2.2 保持 `training` 查询应用现有结构化 hard filters，包括发布态、section、器械、场地、肌群、难度、分类、动作力学、目标标签、风险标签、`q`、`requiredExerciseIds` 和 `excludeExerciseIds`。
- [ ] 2.3 让 `warmup` / `stretch` 查询只应用发布态、section、器械、场地、肌群、`requiredExerciseIds` 和 `excludeExerciseIds` hard filters。
- [ ] 2.4 确保 `warmup` / `stretch` 查询中传入的 `level`、`force`、`mechanic`、`category`、`goalTag`、`riskTag` 和 `q` 不进入数据库 `where`，并被记录为未作为 hard filter 使用。
- [ ] 2.5 确保 policy 在查询前由 section 决定，不由查空结果、用户原文、`q` 文本、关键词或短句模板触发。
- [ ] 2.6 保持 repository 数据库下推，不回退到 `listExerciseRecords()`、`listAllExercises()`、旧 `searchExercises()` 或全量读取后内存过滤。
...
- [ ] 6.1 如实现阶段改变核心动作查询逻辑，在 `docs/方案变更历史/` 中新增中文方案记录，时间使用上海时间到秒。
- [ ] 6.2 如实现阶段影响后续开发的重要问题，在 `docs/项目演变历程.md` 末尾追加简要记录。
- [ ] 6.3 最终 diff 检查，确认没有混入 `stage-exercise-semantic-retrieval`、pgvector / RAG、无关 UI、无关 prompt 或已有脏文件。
