...
- 在训练开始前播报“第一个动作：xxx”，然后播报“3，2，1”，倒计时结束后才启动训练计时。
...
当 `/training` 载入有效 timeline 后，页面先进入准备阶段，不立即启动训练计时器。语音开关开启时，先播报“第一个动作：xxx”，再按秒播报“3，2，1”；倒计时归零后才启动当前训练步骤的正式计时。

准备倒计时只在当前训练会话首次进入时触发；用户手动跳步或计划数据重新加载时不应重复播放旧会话的准备序列。准备阶段需要暂停训练主计时器、热量估算和动作进度，避免用户刚进入页面时训练已经开始消耗时间。
