...
- 每个动作步骤开始前先播报动作名，然后播报“3，2，1，开始”，倒计时结束后才启动该动作步骤的训练计时。
...
当 `/training` 载入有效 timeline 后，页面先进入第一个动作的准备阶段，不立即启动训练计时器。语音开关开启时，先播报“第一个动作：xxx”，再按秒播报“3，2，1，开始”；倒计时归零后才启动当前动作步骤的正式计时。

后续每次进入新的动作步骤时，也必须先进入准备阶段，播报该动作名和“3，2，1，开始”，再开始该动作计时。休息步骤仍按休息计时执行；当休息结束并切到下一个动作步骤时，再触发下一个动作的准备倒计时。准备阶段需要暂停训练主计时器、热量估算和动作进度，避免用户还没开始动作时训练已经消耗时间。
