...
训练中页面目前已经有音量按钮和完整的训练执行状态，但缺少真实语音反馈。用户训练时通常不方便一直看屏幕，语音播报可以在动作切换、开始准备和倒计时阶段提供更直接的节奏提示。
...
- 播报训练开始准备提示，包括需要做哪些动作，以及“3，2，1开始”。
- 播报动作切换提示，包括当前动作、组数目标和“下一组动作”。
- 倒计时阶段播放“嘟嘟声”式提示音，用于最后倒计时或关键节奏提示。
...
- `workout-session-voice-broadcast`: 覆盖训练中页面的语音播报开关、默认发声、训练流程播报、倒计时提示音和本地偏好保存。
