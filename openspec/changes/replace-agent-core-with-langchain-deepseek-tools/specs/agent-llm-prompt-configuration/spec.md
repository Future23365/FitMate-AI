...
**Reason**: 该要求绑定旧 `LlmPlanner` prompt 配置。迁移后仍要求集中配置，但对象变为 LangChain agent prompt 和 DeepSeek native tool calling 配置。

**Migration**: 使用本 change 中 `LangChain 模型可见合同必须描述 native Tool Calling 边界` 与 `agent-runtime-configuration` 的集中配置要求替代。

### Requirement: Prompt 配置必须保持模型供应商边界
**Reason**: 旧 requirement 描述的是 `DeepSeekModelAdapter` 和 `PlannerInput` 边界。迁移后 provider 边界由 LangChain model factory / provider adapter 承担。

**Migration**: LangChain runtime 不得把 DeepSeek provider payload 泄漏到业务 tool；provider 配置仍集中管理。

### Requirement: 默认 prompt 必须只描述通用 AgentAction 合同
**Reason**: 旧 `AgentAction` 合同将被删除，默认 prompt 不再描述 `tool_call` / `final_answer` / `ask_user` JSON 输出格式。

**Migration**: 默认 prompt 描述 LangChain / DeepSeek native Tool Calling 边界、最终回答边界、结构化输出校验和非医疗能力边界。

### Requirement: Prompt 配置不得改变 /api/chat 业务行为
**Reason**: 本 change 的目标正是改变 `/api/chat` 内部 Agent runtime 行为，从旧 self-hosted AgentAction loop 切到 LangChain runtime。

**Migration**: 外部 `/api/chat` 请求和 NDJSON 用户可见响应尽量保持稳定；内部模型输入和 Agent loop 合同按新规格迁移。

### Requirement: Prompt 文档必须指向当前真实入口
**Reason**: 当前真实入口将从旧 prompt 配置入口迁移到 LangChain agent prompt / model factory / tool catalog。

**Migration**: 实现阶段必须更新文档，使其指向新 LangChain prompt 和 tool model-visible contract 入口。
