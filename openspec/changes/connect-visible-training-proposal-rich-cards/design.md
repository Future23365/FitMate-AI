...
当前生产聊天主链已经通过 `final_answer.visibleOutputs[]` 输出 `visibleTrainingProposal`，并由服务端 validator / renderer / fact bridge 保证结构化训练事实、用户可见事件和跨轮引用一致。前端 `features/chat/components/chat-page.tsx` 目前把 `visible_output` 渲染成轻量 `VisibleTrainingProposalPanel`，只能展示基础动作列表和处方摘要。

项目里已有三张用户体验更完整的训练富卡片：`ExerciseRecommendationCard`、`WorkoutRoutineDraftCard` 和 `WorkoutPlanDraftCard`。这些卡片已经覆盖动作详情入口、图片、保存 routine、导入 plan、AI 建议按钮、历史回放补详情等交互。新链路要复用它们，但不能把旧卡片 state 重新变成新事实源，也不能要求模型为了前端展示输出旧 draft 结构。
...
新增前端 adapter，例如 `features/chat/lib/visible-training-proposal-cards.ts`。adapter 接收 `ChatVisibleOutput`、assistant 文本、`assistantSuggestions` 和可选动作详情缓存，返回三类派生视图之一：
...
### 2. `payload` 管事实，`content` 和详情接口管展示
...
### 3. 新消息优先 `visibleOutputs`，旧状态只做历史兼容
...
2. 将聊天气泡 `VisibleTrainingProposalPanel` 替换为富卡片 renderer；保留轻量 panel 作为内部 fallback 或删除无用路径。
