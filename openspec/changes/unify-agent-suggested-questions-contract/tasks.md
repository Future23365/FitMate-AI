...
- [ ] 1.2 清理或迁移 `final_answer.assistantSuggestions`、`ask_user.suggestions`、前端 `suggestedReplies` 等旧字段的新生产写入路径。
- [ ] 1.3 保留短期兼容读取：旧 `assistantSuggestions`、`suggestedReplies`、`suggestions` 和旧 `assistant_suggestions` 可以归一化为 `suggestedQuestions`。
...
- [ ] 3.2 更新 chat client NDJSON parser，优先消费 `suggested_questions`，并兼容旧 `assistant_suggestions`。
...
- [ ] 4.1 更新聊天历史读写：新消息保存 `suggestedQuestions`，旧字段读取时归一化到 `suggestedQuestions`。
- [ ] 4.2 更新服务端聊天历史投影，避免新写入继续产生 `suggestedReplies` 或复杂 `assistantSuggestions` 主路径。
- [ ] 4.3 用 `rg` 检查 `assistantSuggestions`、`suggestedReplies`、`assistant_suggestions`、`suggestions` 残留，区分兼容读取、非本链路业务字段和应迁移的新主路径。
- [ ] 4.4 若推荐卡片或 workout patch 仍需要复杂对象按钮，明确它们不属于聊天主链 `suggestedQuestions` 合同，并避免重复展示。

## 5. 测试与验证

- [ ] 5.1 更新 AgentAction schema / parser 测试，覆盖 `final_answer.suggestedQuestions` 和 `ask_user.suggestedQuestions`。
- [ ] 5.2 更新 Response Renderer 测试，断言新主路径输出 `suggested_questions`，旧字段兼容不会重复展示。
...
- [ ] 5.6 更新历史兼容测试，覆盖旧 `suggestedReplies` / `assistantSuggestions` / `assistant_suggestions` 归一到 `suggestedQuestions`。
...
- [ ] 6.3 更新相关 prompt / chat contract 文档中关于建议回复、建议提问和旧字段迁移的说明。
