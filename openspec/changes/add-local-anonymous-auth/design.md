...
- 除 `POST /api/auth/local-anonymous` 这种匿名会话 bootstrap 接口外，现有 `app/api` Route Handler 都通过统一服务端 auth context 获取 `userId`，继续沿用现有 `userId` 数据隔离。
...
服务端 token payload 包含匿名主体标识、签发时间、过期时间、版本和随机 nonce，并使用 `FITMATE_LOCAL_AUTH_SECRET` 做 HMAC 签名。建议结构如下：
...
其中 `sub` 是匿名主体 id，对应 `UserIdentity(provider = "anonymous").providerAccountId`，不是数据库内部 `User.id`。服务端只在签名有效、版本有效、未过期、匿名主体存在且能映射到 `UserIdentity(provider = "anonymous")` 时返回 `CurrentUser`。token 有效期固定为 `180 days`；过期后前端清理本地凭证并重新展示匿名登录弹窗。

`FITMATE_LOCAL_AUTH_SECRET` 是服务端签发和校验匿名 token 的密钥。非本地环境必须显式配置；本地开发环境可以使用固定开发 secret 并输出 warning，避免每次启动生成随机 secret 导致同一浏览器身份不可预期地失效。缺少生产 secret 时，匿名会话接口和需要鉴权的 API 必须返回配置错误，而不是静默降级到 `local-demo-user`。

取舍：

- 相比直接保存 `userId`，签名 token 可以防止客户端伪造任意用户，也让匿名身份和内部用户主键保持解耦。
...
- `LocalAuthProvider` 必须包住 `AppSidebar` 和页面内容，避免侧栏历史记录、页面数据请求或开发 trace 页面在 auth 状态为 `checking` / `unauthenticated` 时提前请求 API。
- 设置页新增“重置本地用户”入口：点击后清理本地匿名凭证，将 auth 状态切换为未认证，并重新展示匿名登录弹窗。该操作只影响当前浏览器本地身份，不删除服务器上旧匿名用户的数据。

### 3. `getCurrentUser()` 改成基于请求上下文解析

当前 `getCurrentUser()` 无参数，导致服务层可以在没有请求上下文时隐式得到固定用户。改造后采用显式 request auth context：

- Route Handler 从 `Request` 读取 token 并调用 `requireCurrentUser(request)`。
- Route Handler 将 `CurrentUser`、`userId` 或等价请求上下文显式传给业务服务。
- 服务层接收 `CurrentUser` / `userId` / `ServerRequestContext`，不直接解析 token。
- 禁止服务层在缺少 auth context 时回退到 `local-demo-user`。

为了控制改动面，可以保留一个兼容函数名，但其实现必须要求调用方提供请求上下文，或者仅用于显式测试/脚本上下文。正常业务路径必须逐步迁移为显式接收 `userId` 或 `CurrentUser`，减少隐式全局依赖。

### 4. 私有 API 统一未认证响应

除 `POST /api/auth/local-anonymous` 这种创建或恢复匿名会话的 bootstrap 接口外，现有 `app/api` Route Handler 都必须在缺少有效凭证时返回统一错误，例如：
...
前端请求层收到 `401 unauthenticated` 时清理本地匿名凭证并通知 auth provider 重新展示弹窗。该处理不能只放在 `throwOnError` 分支；即使调用方设置 `throwOnError: false` 或 `responseType: "raw"`，只要响应状态为 `401` 且响应体错误码为 `unauthenticated`，也必须触发凭证清理和重新登录流程。流式聊天请求如果收到未认证响应，也必须先完成 auth 失效处理，再把响应交给调用方判断。
...
- [Risk] 所有现有 API 都改为需要匿名凭证后，动作库和开发 trace 页面也必须等待 auth ready。→ Mitigation: `LocalAuthProvider` 包住侧栏、页面和开发页入口；所有前端 API 调用统一走可注入凭证的请求层，避免零散 `fetch` 漏带 token。
...
3. 改造 `clientRequest()` 自动等待/注入匿名 token，并在所有响应路径处理 `401 unauthenticated`。
4. 改造现有 `app/api` Route Handler，先解析当前用户，再传入服务层或请求上下文；`POST /api/auth/local-anonymous` 是唯一允许无 token 创建匿名会话的 bootstrap 入口。
5. 移除正常请求路径对 `local-demo-user` 的依赖，保留测试和脚本所需的显式 mock/fixture。
6. 在设置页新增重置本地用户入口。
7. 更新数据库设计、架构说明和方案变更历史。
8. 运行单元测试、类型检查，并按影响面运行构建。
