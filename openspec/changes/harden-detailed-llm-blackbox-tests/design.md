...
- runner 直接复用服务端聊天编排函数并手工维护会话状态，和真实首页通过 `/api/chat` 请求、保存会话、写入 artifact、下一轮再从数据库读取 recent artifact 的链路不完全一致。
...
- 让完整/详细 LLM 测试更贴近真实首页聊天路径，优先验证 `/api/chat`、会话保存、artifact 持久化、同一测试用户权限隔离和后续引用读取。
...
完整/详细测试应新增或重构 runner，使每轮请求尽量靠近真实首页行为：构造 `conversationId`、`responseMessageId`、`latestUserMessage`、`conversationSummary`、`thinkingEnabled`，以测试专用用户调用 `/api/chat`，处理 NDJSON stream，然后按首页会话保存边界保存该轮用户消息、assistant 消息、conversation summary、conversation context 和可见卡片 payload，下一轮再继续。
...
### 2. 会话保存是 artifact 链路的一部分

`/api/chat` 负责读取当前用户可访问的 recent artifact summary 并返回 stream；`ConversationArtifact` 和 `ArtifactIndex` 的真实写入发生在会话保存链路。详细 runner 因此不能只消费 `/api/chat` stream 后在内存中拼 state，而必须复用或模拟首页的保存边界，使生成卡片后的下一轮请求能从数据库读取 recent artifact。

取舍：在测试 runner 内直接调用 artifact service 写入数据会更短，但会绕过真实聊天历史保存结构，无法发现 sourceMessageId、message metadata、userId 隔离和 active revision 相关问题。详细套件应覆盖这些边界；基础套件仍可保持轻量。

### 3. 引用类用例必须依赖真实 artifact
...
### 4. 测试用户和环境 preflight 必须显式

详细 runner 需要在运行前确认测试专用用户、会话保存接口、数据库连接、必要 migration、`ConversationArtifact` / `ArtifactIndex` 表和基础 seed 数据可用。缺少 `DEEPSEEK_API_KEY` 时保持跳过真实模型；缺少数据库或 schema 时应以环境失败或跳过摘要明确写入报告，不得退回 mock、旧快照或手工 recent artifact。

取舍：preflight 会增加 runner 复杂度，但它能把“测试环境不可用”和“模型/业务行为失败”分开，避免详细报告误导后续修复方向。
...
最终报告状态应保持可聚合：`passed` 表示卡片类型和语义目标都通过；`failed` 表示 P0/P1/P2 中任一自动断言失败；`skipped` 表示缺少 key、preflight 未满足或前序轮次失败导致未执行；`needs_review` 仅用于 P3 或自动断言无法稳定判断的内容质量复核，不应计为通过。
...
- token 预估口径、实际 token 汇总、与上一份真实报告的偏差，以及没有可用真实报告时使用的 fallback 估算来源。
...
- [Risk] `/api/chat` 请求成功但会话保存失败会让下一轮引用测试失真。→ Mitigation: 把会话保存结果、artifact 写入结果和下一轮 recent artifact 读取结果都纳入报告诊断。
