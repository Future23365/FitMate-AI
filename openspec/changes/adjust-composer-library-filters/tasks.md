...
- [x] 2.1 将右侧动作库当前横向 `category` chip 区域改为热身、训练、拉伸三个阶段入口，点击后更新 `selectedSection`。
...
- [x] 3.1 验证选择热身、训练、拉伸后，从右侧动作库添加动作会进入对应 section。
- [x] 3.2 验证分类下拉筛选会请求 `/api/exercises` 并携带 `category` 参数。
- [x] 3.3 验证居家条件下拉筛选会请求 `/api/exercises` 并携带 `homeRequirement` 参数。
- [x] 3.4 验证清空筛选会移除搜索词、分类、肌群、器械、难度和居家条件，但不改变当前选中的新增阶段。

## 4. 检查

- [x] 4.1 运行 `npm run lint`。
- [x] 4.2 运行 `npm run typecheck`。
- [x] 4.3 运行 `npm test` 或与动作库筛选相关的测试，并记录结果。
