...
最新 `codex_logs/ai_trace_log.js` / `ai_trace_texts.jsonl` 链路审计显示，现有 AI trace 能复盘 tool call、tool 执行失败、token 和 tool result 的模型可见摘要，但不能证明 system prompt、tool description、schema description 和 finalization 合同是否完整进入 provider request。继续只基于当前 trace 的 `requestSummary.messagePreviews` 排查，会把“提示词未生效”和“提示词生效但模型未遵守”混在一起。
...
- 新增模型可见输入审计字段：记录 system prompt、tool description、schema description、finalization tool、messages 和工具可用性的来源、长度、hash / fingerprint 和完整性状态，避免把不完整 trace 当作真实模型输入。
...
- 保持生产 `/api/chat`、DeepSeek provider 调用、业务 tool handler、数据库事实和用户可见 NDJSON 输出不变；如需补充 runtime trace 字段，只能作为 dev-only 可观测性补强，不改变模型调用行为。
...
- 新增 trace 链路准确性约束：Shadow Probe 不得把现有 AI trace 的 `requestSummary.messagePreviews`、`toolNames` 或导出日志标题说明当作完整模型可见输入；从 `/dev/ai-traces` 导入真实 trace 必须先证明 system prompt 和 tool schema 快照存在，否则只能标记为不完整来源。
