...
- [ ] 1.5 区分 `history` / `artifact` 中的用户确认约束和旧模型生成字段，只有前者可触发 hard fail。
- [ ] 1.6 保留用户明确避免动作或具体训练禁忌的 hard fail；未主动提出或未确认的伤病限制不得作为 hard fail。
- [ ] 1.7 确保用户明确要求重复当前 routine 时，`repeat_previous_routine` / `repeat_same_routine_with_progression` 计划不会因重复训练日或连续负荷失败。
...
- [ ] 3.4 增加测试覆盖旧模型生成或未确认 artifact 字段不触发 hard fail，用户确认的 history / artifact 约束仍可触发 hard fail。
- [ ] 3.5 增加测试覆盖用户明确避免动作或具体训练禁忌仍可 hard fail，未主动提出或未确认的伤病限制不 hard fail。
- [ ] 3.6 增加测试覆盖用户明确时长或频率被违反时仍进入可恢复契约失败。
- [ ] 3.7 更新 `tests/ai-workout-plan-service.test.ts`，覆盖 warning 不触发失败恢复、未知 error 不被 warning 误分类。
- [ ] 3.8 按需更新聊天服务测试，覆盖“今天明天后天都练这个”可以返回计划卡片。
