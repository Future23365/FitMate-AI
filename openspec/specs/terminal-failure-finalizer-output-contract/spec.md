...
系统 SHALL 保持主 Agent validator、terminal output validator、ResourceStore、Policy Guard、Response Renderer 和权限隔离边界。放宽 finalizer 输出文案校验 MUST NOT 使未通过校验的结构化输出被渲染、保存或作为事实来源。

#### Scenario: finalizer 只输出普通聊天事件

- **WHEN** 主 Agent 因 `visibleOutputs[]`、terminal reference 或 resource grounding 校验失败而进入 terminal failure finalizer
- **AND** finalizer 返回 shape 合法输出
- **THEN** 响应 MUST NOT 输出被拒绝的 `visible_output` 事件
- **AND** 系统 MUST NOT 持久化被拒绝的 visible output
- **AND** 系统 MUST NOT 把 finalizer `content` 或 `suggestedQuestions` 注册为 consumable resource
- **AND** trace MUST 区分 finalizer reply 和主 Agent 成功 `final_answer`
